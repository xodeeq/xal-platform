...
**Feed edges declare two secret names.** The consumer declares `XAL_FEED_CREDENTIAL__<publisher>`;
the publisher declares `XAL_FEED_ALLOWLIST` (required when the feed profile is on). Generated by
composition; interim: the publisher's provisioning script (ADR-0005).
