...
| [0006](0006-repo-seeding.md) | Repo seeding: a new service repo begins with its gate. The scaffold grows a `common/` + `lang/<language>/` split and a seeder (`scaffold/seed-service.sh`); `xcos-core` is disqualified because a plugin is not resolvable from a plain CI `run:` step. Language comes from the service's own ADR-0001 and has no default; installing the Claude GitHub App and setting its secret stay human-only. Closes the 2026-08-18 seeding-gap ledger entry and platform session 02's "the scaffold ships no gate script". No `spec/` change, so no `VERSION` bump | Accepted |
| [0007](0007-claude-config-sync-boundary.md) | The `.claude/` sync boundary: a plugin-distributed normative core plus an explicitly service-owned overlay; `sync/manifest` is NOT extended to `.claude/`, and `scaffold/.claude/agents/` is where a shared agent definition goes. Records at platform level what xal-company ADR-0004 built and ADR-0006 ruled adjacently, and lifts the `xal-company/CLAUDE.md` block on the four service-repo subagents as a distribution question. No `spec/` change, so no `VERSION` bump | Accepted |
| [0008](0008-marketplace-sources-are-remote.md) | Marketplace sources are remote and pinned: a `"source": "directory"` registration is a defect, a private marketplace declares a least-privilege read credential as a gate input, and `.xal/gate-inputs` gains a sixth `expires` field that `check-gate-inputs.sh` enforces. Makes ADR-0007's distribution model work off the authoring machine — including in a headless CI checkout, which the pipeline driver needs. No `spec/` change, so no `VERSION` bump | Accepted |
